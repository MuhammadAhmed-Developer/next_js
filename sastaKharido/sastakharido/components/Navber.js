  ...
  return (
    <div className='flex  flex-col md:flex-row md:justify-start   justify-center items-center  my-2 shadow-md'>
      ...
      <div ref={ref} className="sidebar h-full  w-72 sideCart absolute right-0 top-0 bg-green-100 p-10 transform transition-transform translate-x-full">
        ...
        <ol className='list-decimal'>
          ...
          {Object.keys(cart).map((k) => {return <li key={k}>
              <div className="item flex my-5">
                ...
                <div className=' flex items-center justify-center'><AiFillPlusSquare className='cursor-pointer mx-2 text-2xl  text-green-400' /><span className='text-2xl mb-1'>{cart[k].qty}</span>  <AiFillMinusSquare onClick={()=>{removeToCart(cart[k].itemCode, 1, cart[k].price , cart[k].name, cart[k].size, cart[k].variant )}} className='cursor-pointer mx-2 text-2xl text-red-400' /></div>
              </div>
            </li>
          })}
        ...
        <div className="flex">
          <button className="flex text-white bg-green-500 border-0 py-2 px-2 focus:outline-none hover:bg-green-600 rounded text-sx"><BsFillBagCheckFill className='mx-1 mt-' /> CheckOut</button>
